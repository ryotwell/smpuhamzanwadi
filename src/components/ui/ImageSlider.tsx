...
export const ImageSlider: React.FC<ImageSliderProps> = ({
    ...
    // Reset to 0 if slides change or current is out of bounds
    React.useEffect(() => {
        setCurrent(0);
    }, [slides]);
        ...
        <section className="relative w-full max-w-4xl mx-auto mt-10 md:rounded-3xl overflow-hidden shadow-2xl group bg-white dark:bg-gray-900 transition-colors duration-300">
            ...
            <div className="relative h-[340px] sm:h-[420px] md:h-[500px] transition-all duration-500">
                {slides.map((slide, idx) => (
                    <img key={idx} src={slide.image} alt={slide.title} className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${idx === activeIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`} />
                ))}
                {/* Overlay gradient */}
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-40">
                {slides.map((_, idx) => (
                    <button
                        ...
                        className={`w-3 h-3 rounded-full border-2 border-white dark:border-white transition-all duration-200 ${activeIndex === idx ? 'bg-white dark:bg-white' : 'bg-white/40 dark:bg-white/30'}`}
                        style={{ aspectRatio: "1/1" }}
                    />
                ))}
            </div>
